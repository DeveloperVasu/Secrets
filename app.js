...
// npm install dotenv express body-parser ejs mongoose express-session passport passport-local passport-local-mongoose
...
app.get("/secrets", function(req, res) {
  ...
  a cached page (like the secrets one)
  you can go back by pressing the back button on the browser, the code to fix it is the one below
  so the page will not be cached
  */
...
app.post("/register", function(req, res) {
  ...
  User.findOne({
    ...
    if (ferr) {
      ...
      */
      User.register({username: req.body.username}, req.body.password, function(rErr, rUser) {
        if (rErr) {
...
//app is crashing if we are using same user id again to register
